...
app.get("/a/b/c/d/:id",(req,res)=>{
    console.log(req.body.a," ",req.body.b," ",req.body.c," ",req.body.a);
})
...
app.get('/yourteam/:tcode',auth,(req,res)=>{
    ...
    let c = req.params.tcode;
    team.find({tcode: req.params.tcode}).then(function(list){
        project.find({codec: c}).then(function(pot){
            console.log(req.user.username," opened team ",list[0].team_name);
            res.render("yourteam",{
...
app.post("/yourteam/:tcode", auth,(req,res)=>{
    ...
    const f= feed.save()
    console.log("User created project ",req.body.pname);
    let c = req.params.tcode;
    team.find({tcode: c}).then(function(list){
        project.find({codec: c}).then(function(pot){
            res.render("yourteam",{
                data: req.user, team: list, pro: pot})
        })
...
app.post("/yourteam/update/:project/:user",auth, (req,res)=>{

    project.findOne({pro_name: req.params.project}).then(function(lst){
        const code = lst.codec;
        team.find({tcode: code}).then(async(list)=>{
                let count=0;
                if(list[0].user1)
                {
                    count++;
                ...
                if(req.user.username==list[0].user1)
                {
                    const up = lst.pro_progress+(100/count); 
                    const a = await project.findOneAndUpdate({pro_name: req.params.project},{pro_progress:up},{
                        new: true
                      });
                    const b = await project.findOneAndUpdate({pro_name: req.params.project},{pu1:true},{
                        new: true
                      });
                }
                if(req.user.username==list[0].user2)
                {
                    const up = lst.pro_progress+(100/count); 
                    const a = await project.findOneAndUpdate({pro_name: req.params.project},{pro_progress:up},{
                        new: true
                      });
                    const b = await project.findOneAndUpdate({pro_name: req.params.project},{pu2:true},{
                        new: true
                      });
                }
                if(req.user.username==list[0].user3)
                {
                    const up = lst.pro_progress+(100/count); 
                    const a = await project.findOneAndUpdate({pro_name: req.params.project},{pro_progress:up},{
                        new: true
                      });
                    const b = await project.findOneAndUpdate({pro_name: req.params.project},{pu3:true},{
                        new: true
                      });
                }
                if(req.user.username==list[0].user4)
                {
                    const up = lst.pro_progress+(100/count); 
                    const a = await project.findOneAndUpdate({pro_name: req.params.project},{pro_progress:up},{
                        new: true
                      });
                    const b = await project.findOneAndUpdate({pro_name: req.params.project},{pu4:true},{
                        new: true
                      });
                }
        })
    ...
    project.findOne({pro_name: req.params.project}).then(function(lst){
        ...
        const a = "/yourteam/"
        const b = a.concat(code)
        // console.log("URL:",b);

        team.find({tcode: code}).then(function(list){
                project.find({pro_name: req.params.pro_name}).then(function(pot){
                        res.render("yourteam",{
                                data: req.user, team: list, pro: pot})
                            })
                        })
...
app.get("/account/admin", auth,(req, res) => {
    User.find().then(function(lists){
        team.find().then(function(teams){
            project.find().then(function(pro){
                // console.log("PRO:",pro);
                // console.log("Team:",teams);
                res.render("admin", { data: req.user,datas: lists,team: teams,pros: pro });  
            })
        })
    })
});
...
app.get("/account/admin/project/:id",auth,(req,res)=>{
    if(req.user.stafflogin==true)
    {
